...
class FluxGenerator:
    def __init__(self, model_name: str, device: str, offload: bool):
        ...
        # Load your models here (omitting for simplicity)
    ...
    @torch.no_grad()
    def generate_image(self, width, height, num_steps, guidance, seed, prompt, init_image=None, image2image_strength=0.0, add_sampling_metadata=True):
        seed = int(seed)
        if seed == -1:
            seed = None
...
# Initialize the FluxGenerator
generator = FluxGenerator(model_name="flux-schnell", device="cuda", offload=False)
...
def generate_image():
    ...
    image2image_strength = data.get('image2image_strength')
    add_sampling_metadata = data.get('add_sampling_metadata')

    # Call the generate_image function with the extracted parameters
    ...
    img_io = io.BytesIO()
    image.save(img_io, 'PNG')  # Save the image as PNG
    ...
    return jsonify({
        ...
        "seed": seed  # or any other additional data
    })
